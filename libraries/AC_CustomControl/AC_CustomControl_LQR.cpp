...
// [T1 T2 T3] = -k_gain*[v1 v2 v3 p q r]
   ...
   // _integralZ += 0.005*attitude_error_angle.z*_dt;
    _integralX += 0*attitude_error_angle.x*_dt;
    _integralY += 0*attitude_error_angle.y*_dt;
    _integralZ += 0*attitude_error_angle.z*_dt;
    ...
    motor_out.x = -_integralX - 0.5477*attitude_error_angle.x - 0.2093*gyro_latest.x;

    motor_out.y = -_integralY - 0.5477*attitude_error_angle.y - 0.2093*gyro_latest.y;
   
    motor_out.z = -_integralZ - 0.3162*attitude_error_angle.z - 0.1384*gyro_latest.z;


    

    cout << "Motor Output - X: " << motor_out.x << ", Y: " << motor_out.y << ", Z: " << motor_out.z << endl;
